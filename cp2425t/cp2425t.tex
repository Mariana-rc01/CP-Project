...
A função \ensuremath{\Varid{convolve}} foi implementada como um anamorfismo de listas (\ensuremath{\Conid{List}}), já que a função é responsável por criar a lista resultante da convolução de 2 listas fornecidas, \ensuremath{\Varid{l1}} e \ensuremath{l_2 }.
Esta vai construindo recursivamente a lista resultado recorrendo à função \ensuremath{\anaList{\cdot }}, aplicando sucessivamente  às duas listas o gene \ensuremath{\Varid{g}} que, neste caso, está realmente construído na função \ensuremath{\Varid{anaGene}}.

A função \ensuremath{\Varid{anaGene}} começa por verificar se o índice \ensuremath{\Varid{i}} (inicialmente 0) ainda se encontra dentro dos limites da convolução, isto é, se \ensuremath{\Varid{i}\geq \Varid{m}\mathbin{+}\Varid{n}\mathbin{-}\mathrm{1}}. Se isto se verificar, será retornada uma lista vazia, visto que a convolução terminou, não sendo adicionado nenhum valor novo à lista final.
Caso contrário, será calculado o elemento seguinte da lista resultado, realizando efetivamente o próximo passo da convolução: \ensuremath{\Varid{sum}\mathbin{\$}\Varid{zipWith}\;(\mathbin{*})\;\Varid{l1}\;(\map \;(\lambda \Varid{j}\to \Varid{access}\;(l_2 ,(\Varid{i},\Varid{j})))\;[\mskip1.5mu \mathrm{0}\mathinner{\ldotp\ldotp}(\Varid{m}\mathbin{-}\mathrm{1})\mskip1.5mu])}

Além disso, neste cálculo, é verificado se o índice \ensuremath{\Varid{i}\mathbin{-}\Varid{j}} de \ensuremath{l_2 } é válido, através da função \ensuremath{\Varid{access}}, onde duas condições são confirmadas: \ensuremath{\Varid{i}\mathbin{-}\Varid{j}} tem de ser não negativo (\ensuremath{\uncurry{(\mathbin{>})}\comp (\underline{\mathrm{0}}\times\uncurry{(\mathbin{-})})}) e menor que o comprimento de \ensuremath{l_2 } (\ensuremath{\uncurry{(\leq )}\comp (\length \times\uncurry{(\mathbin{-})})}).

A função \ensuremath{\Varid{convolve}} é representada pelo seguinte diagrama:

\begin{eqnarray*}
\xymatrix@C=2cm{
    \ensuremath{\mathbb{Z}}^* \times \ensuremath{\mathbb{Z}}^*
           \ar[d]_-{\ensuremath{\anaList{\Varid{g}}}}
            \ar[r]^-{\ensuremath{\Varid{g}}} 
&
    \ensuremath{\mathrm{1}\mathbin{+}\mathbb{Z}} \times (\ensuremath{\mathbb{Z}}^* \times \ensuremath{\mathbb{Z}}^*)
           \ar[d]^{\ensuremath{\Varid{id}\mathbin{+}(\Varid{id}} \times \ensuremath{\anaList{\Varid{g}}})}
\\
     \ensuremath{\mathbb{Z}}^*
            \ar@/_/[r]_-{\ensuremath{\Varid{out}}_\ensuremath{\Conid{List}}} 
...
A implementação da função \ensuremath{\Varid{convolve}} realizada foi a seguinte:
...
Para definir o catamorfismo monádico de \ensuremath{\Conid{Expr}}, a função \ensuremath{\Varid{mcataExpr}}, começamos por entender o papel fundamental da função \ensuremath{\Varid{dl}}, que é responsável por transformar a estrutura de \ensuremath{\Conid{Expr}} num contexto monádico. Esta transformação permite que o processamento das expressões ocorra dentro de um monad.

A função \ensuremath{\Varid{dl}} é definida como:
...
- No caso de um valor do tipo \ensuremath{\Varid{a}}, a função simplesmente o encapsula no monad utilizando \ensuremath{\Varid{return}\comp i_1}.

- No caso de um valor do tipo \ensuremath{\Varid{b}}, o mesmo ocorre, utilizando \ensuremath{\Varid{return}\comp i_2\comp i_1}.

- No caso de uma operação (\ensuremath{\Conid{Op}}) acompanhada por uma lista de valores monádicos (\ensuremath{[\mskip1.5mu \Varid{m}\;\Varid{c}\mskip1.5mu]}), é usada a função auxiliar \ensuremath{\Varid{aux}}.

Dentro de \ensuremath{\Varid{aux}}, a função \ensuremath{\Varid{lamb}} é utilizada para distribuir o monad pela lista. Em seguida, o resultado é encapsulado novamente na estrutura esperada pelo tipo \ensuremath{\Varid{m}\;(\Varid{a}+(\Varid{b}+(\Conid{Op},\Varid{m}\;[\mskip1.5mu \Varid{c}\mskip1.5mu])))}.

A função \ensuremath{\Varid{mcataExpr}}, que define o catamorfismo monádico propriamente dito, é então definida da seguinte forma:
