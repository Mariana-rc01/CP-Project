...
\>[B]{}\Varid{convolve}\mathbin{::}\Conid{Num}\;\Varid{a}\Rightarrow [\mskip1.5mu \Varid{a}\mskip1.5mu]\to [\mskip1.5mu \Varid{a}\mskip1.5mu]\to [\mskip1.5mu \Varid{a}\mskip1.5mu]{}\<[E]%
\\
\>[B]{}\Varid{convolve}\;\Varid{l1}\;l_2 \mathrel{=}\anaList{\Varid{convolveGene}\;\Varid{l1}\;l_2 }\;\mathrm{0}{}\<[E]%
\\[\blanklineskip]%
\>[B]{}\Varid{convolveGene}\mathbin{::}\Conid{Num}\;\Varid{a}\Rightarrow [\mskip1.5mu \Varid{a}\mskip1.5mu]\to [\mskip1.5mu \Varid{a}\mskip1.5mu]\to \Conid{Int}\to ()+(\Varid{a},\Conid{Int}){}\<[E]%
\\
\>[B]{}\Varid{convolveGene}\;\Varid{l1}\;l_2 \mathrel{=}\Varid{cond}\;(\geq \Varid{m}\mathbin{+}\Varid{n}\mathbin{-}\mathrm{1})\;(\underline{\cdot }\mathbin{\$}i_1\;()){}\<[E]%
\\
\>[B]{}\hsindent{21}{}\<[21]%
...
Outra abordagem para a implementação da convolução de listas está representada pela função com \ensuremath{\Varid{convolve2}}. Esta abordagem
utiliza tem como ideia base adicionar zeros ao início da lista \ensuremath{l_2 } antes de iniciar o processo de determinar os elementos da convolução.
A função auxiliar \ensuremath{\Varid{addzeros}} realiza essa operação, ajusta o comprimento de \ensuremath{l_2 } para permitir que o cálculo da 
...
Embora a função \ensuremath{\Varid{convolve2}} funcione corretamente e forneça os resultados corretos, optamos pela implementação baseada em \ensuremath{\Varid{convolve}}
por alguns motivos. Primeiro, \ensuremath{\Varid{convolve}} não exige a manipulação inicial da lista de entrada, como a adição de zeros.
...
Além disso, a implementação da \ensuremath{\Varid{convolve}} reflete de forma mais direta a definição matemática da convolução, integrando 
...
\column{7}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\Varid{convolve2}\mathbin{::}\Conid{Num}\;\Varid{a}\Rightarrow [\mskip1.5mu \Varid{a}\mskip1.5mu]\to [\mskip1.5mu \Varid{a}\mskip1.5mu]\to [\mskip1.5mu \Varid{a}\mskip1.5mu]{}\<[E]%
\\
\>[B]{}\Varid{convolve2}\;\Varid{l1}\;l_2 \mathrel{=}\anaList{\Varid{anaGene2}\;\Varid{l1}}\;(\Varid{addzeros}\;l_2 ){}\<[E]%
\\
\>[B]{}\hsindent{5}{}\<[5]%
\>[5]{}\mathbf{where}{}\<[E]%
\\
\>[5]{}\hsindent{2}{}\<[7]%
\>[7]{}\Varid{addzeros}\;\Varid{l}\mathrel{=}\Varid{replicate}\;(\length \;\Varid{l1}\mathbin{-}\mathrm{1})\;\mathrm{0}\mathbin{+\!\!+}l_2 {}\<[E]%
\\[\blanklineskip]%
\>[B]{}\Varid{anaGene2}\mathbin{::}\Conid{Num}\;\Varid{a}\Rightarrow [\mskip1.5mu \Varid{a}\mskip1.5mu]\to [\mskip1.5mu \Varid{a}\mskip1.5mu]\to ()+(\Varid{a},[\mskip1.5mu \Varid{a}\mskip1.5mu]){}\<[E]%
\\
\>[B]{}\Varid{anaGene2}\;\Varid{l1}\;l_2 {}\<[E]%
\\
\>[B]{}\hsindent{5}{}\<[5]%
\>[5]{}\mid \Varid{null}\;l_2 \mathrel{=}i_1\;(){}\<[E]%
\\
\>[B]{}\hsindent{5}{}\<[5]%
\>[5]{}\mid \Varid{otherwise}\mathrel{=}i_2\;(\Varid{sum}\;(\Varid{zipWith}\;(\mathbin{*})\;(\Varid{reverse}\;\Varid{l1})\;(\Varid{take}\;(\length \;\Varid{l1})\;l_2 )),\Varid{tail}\;l_2 ){}\<[E]%
\ColumnHook
\end{hscode}\resethooks
...
\begin{eqnarray*}
\start
\ensuremath{\llparenthesis\, \Varid{g}\,\rrparenthesis\mathrel{=}\llparenthesis\, \Varid{g}\,\rrparenthesis}
...
\begin{eqnarray*}
\start
\ensuremath{\ana{\Varid{g}}\mathrel{=}\ana{\Varid{g}}}
...
\begin{itemize}
\item se a expressão é uma variável (\ensuremath{\Conid{V}\;\Varid{f}}), aplica o \ensuremath{\mathsf{fmap}} para mapear função sobre a outra expressão;
...
Por fim, a instância \ensuremath{\Conid{Monad}} foi definida, a definição \ensuremath{\Varid{return}} é equivalente a \ensuremath{\Varid{pure}} (= \ensuremath{\Conid{V}} = u), cria uma variável.
...
&
    \ensuremath{\Conid{Expr}\;\Conid{B}\;\Conid{A}}
&
    \ensuremath{\Conid{Expr}\;\Conid{B}\;(\Conid{Expr}\;\Conid{B}\;\Conid{A})}
            \ar[l]^-{\ensuremath{\Varid{mu}}}
}
\end{eqnarray*}
...
O componente central dessa definição é a função \ensuremath{\Varid{dl'}}, esta transforma os diferentes casos da estrutura \ensuremath{\Conid{Expr}} para que os
...
A função \ensuremath{\Varid{dl'}} é dividida em três casos principais:

- Para variáveis (\ensuremath{\Varid{a}}), a função simplesmente o encapsula no monad utilizando \ensuremath{\Varid{return}\comp i_1}.

- Para constantes (\ensuremath{\Varid{b}}), o mesmo ocorre, utilizando \ensuremath{\Varid{return}\comp i_2\comp i_1}.

- Para operadores (\ensuremath{\Conid{Op}}) acompanhada por uma lista de valores monádicos (\ensuremath{[\mskip1.5mu \Varid{m}\;\Varid{c}\mskip1.5mu]}), é usada a função auxiliar \ensuremath{\Varid{aux}}.

Dentro de \ensuremath{\Varid{aux}}, a função \ensuremath{\Varid{lamb}} é utilizada para distribuir o monad pela lista de valores monádicos (\ensuremath{[\mskip1.5mu \Varid{m}\;\Varid{c}\mskip1.5mu]}),
de modo a produzir um valor monádico contendo a lista de valores já resolvidos (\ensuremath{\Varid{m}\;[\mskip1.5mu \Varid{c}\mskip1.5mu]}).
Em seguida, o resultado é encapsulado novamente na estrutura esperada pelo tipo \ensuremath{\Varid{m}\;(\Varid{a}+(\Varid{b}+(\Conid{Op},\Varid{m}\;[\mskip1.5mu \Varid{c}\mskip1.5mu])))}.

A função \ensuremath{\Varid{mcataExpr}} utiliza a função \ensuremath{\Varid{dl'}} como parte da transformação recursiva, sendo definida da seguinte forma:
...
Esta definição, assemelha-se à definição do catamorfismo simples, com a diferença de que a função \ensuremath{\Varid{dl}} é usada para distribuir o monad sobre a lista de subexpressões:
\begin{itemize}
\item A estrutura de entrada (\ensuremath{\Conid{Expr}}) é decomposta usando a função \ensuremath{\Varid{outExpr}};
\item A função \ensuremath{\Varid{recExpr}} aplica recursivamente o catamorfismo monádico a todas as subexpressões da estrutura;
\item A função \ensuremath{\Varid{dl'}} reorganiza os valores monádicos produzidos pela decomposição das subexpressões, sendo responsável por lidar com os efeitos monádicos corretamente;
\item Por fim, a álgebra monádica \ensuremath{\Varid{g}} é aplicada para produzir o valor final no contexto monádico.
\end{itemize}
\paragraph{}
4.
\emph{Maps}:
